...
export function CalendarPanel({ dark, selectedDate, onSelectDate, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div className="relative rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-sm animate-slide-up"
        style={{ background: surface, border: `1px solid ${border}` }}>

        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b" style={{ borderColor: border }}>
          <button onClick={prevMonth} className="w-9 h-9 rounded-xl flex items-center justify-center text-xl hover:opacity-70"
            style={{ background: dark ? 'rgba(255,255,255,0.07)' : '#f1f5f9', color: dark ? '#94a3b8' : '#64748b' }}>‹</button>
          <h3 className="font-display font-bold text-lg" style={{ color: dark ? '#f1f5f9' : '#0d1b2a' }}>{monthName}</h3>
          <div className="flex items-center gap-1">
            <button onClick={nextMonth} className="w-9 h-9 rounded-xl flex items-center justify-center text-xl hover:opacity-70"
              style={{ background: dark ? 'rgba(255,255,255,0.07)' : '#f1f5f9', color: dark ? '#94a3b8' : '#64748b' }}>›</button>
            <button onClick={onClose} className="w-9 h-9 rounded-xl flex items-center justify-center text-xl ml-1"
              style={{ background: dark ? 'rgba(255,255,255,0.07)' : '#f1f5f9', color: dark ? '#94a3b8' : '#64748b' }}>×</button>
          </div>
        </div>
        ...
        {/* Footer */}
        <div className="px-5 pb-5 flex items-center gap-4 text-xs" style={{ color: dark ? '#475569' : '#94a3b8' }}>
