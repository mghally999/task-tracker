...
export function CEODashboard({ tasks, stats, notes, currentUser, onlineUsers, dark, onComment, onStatusChange }: Props) {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {onlineUsers.filter(u => u.userId !== currentUser.id).length > 0 && (
        <div className="flex items-center gap-3 px-5 py-3 rounded-2xl"
          ...
          <span className="text-sm font-medium" style={{ color: dark ? '#86efac' : '#166534' }}>
            {onlineUsers.filter(u => u.userId !== currentUser.id).map(u => u.userName).join(', ')} is active right now — changes appear live
          </span>
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-4">
        {([
          { label: 'High Priority', tasks: high, color: '#ef4444', icon: '🔴' },
          { label: 'Medium Priority', tasks: medium, color: '#f59e0b', icon: '🟡' },
          { label: 'Low Priority', tasks: low, color: '#22c55e', icon: '🟢' },
        ] as const).map(({ label, tasks: list, color, icon }) => {
          ...
          return (
            <div key={label} style={cardStyle}>
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="ml-auto font-bold font-display text-2xl" style={{ color }}>{list.length}</span>
              </div>
              <div className="grid grid-cols-2 gap-2 text-xs">
                {Object.entries(counts).map(([status, count]) => (
                  <div key={status} className="flex items-center justify-between px-2 py-1.5 rounded-lg"
                    ...
                    <span style={{ color: dark ? '#64748b' : '#94a3b8' }}>{STATUS_CONFIG[status as any]?.label}</span>
      ...
      <div style={cardStyle}>
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="text-xs mt-1" style={{ color: dark ? '#64748b' : '#94a3b8' }}>
              All active tasks sorted by priority. Click any task to expand details. Leave comments using 💬.
            </p>
          </div>
          <div className="text-xs px-3 py-1.5 rounded-full font-semibold"
            style={{ background: dark ? 'rgba(59,130,246,0.1)' : '#eff6ff', color: '#3b82f6' }}>
            👔 Read Mode
          </div>
        </div>
        ...
        {/* Table header */}
        <div className="grid grid-cols-12 gap-2 px-4 pb-2 text-xs font-bold uppercase tracking-wider"
        ...
        <div>
          ...
          {tasks.filter(t => !t.archived).length === 0 && (
            <div className="text-center py-12">
              ...
              <p style={{ color: dark ? '#475569' : '#94a3b8' }}>No active tasks</p>
            </div>
          )}
        </div>
      </div>
