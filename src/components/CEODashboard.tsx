...
export function CEODashboard({
  ...
  return (
    <div className="space-y-4 sm:space-y-6 animate-fade-in">
      ...
      {/* Priority cards — 1 col mobile, 3 col desktop */}
          ...
          return (
            <div key={label} style={card}>
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="font-semibold text-base" style={{ color: dark ? '#f1f5f9' : '#0d1b2a' }}>{label}</h3>
      ...
      <div style={card}>
        <div className="flex items-start justify-between mb-4 flex-wrap gap-3">
          <div>
            ...
            <p className="text-sm mt-0.5" style={{ color: dark ? '#64748b' : '#94a3b8' }}>
              Full control — edit, archive, delete & comment
            </p>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            <div className="flex items-center gap-1 rounded-xl p-1" style={{ background: dark ? 'rgba(255,255,255,0.05)' : '#f1f5f9' }}>
              ...
              <button style={tabStyle(activeTab === 'priority')} onClick={() => setActiveTab('priority')}>Priority</button>
        ...
        {activeTab === 'all' && (
          active.length === 0
            ...
            : active.map(task => <TaskRow key={task.id} task={task} currentUser={currentUser} canEdit={true} dark={dark} onStatusChange={onStatusChange} onEdit={onEdit} onArchive={onArchive} onRestore={onRestore} onDelete={onDelete} onComment={onComment} />)
        )}
