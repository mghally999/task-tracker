...
export function Navbar({ user, onlineUsers, connected, dark, onToggleTheme, onLogout, view, onToggleView, selectedDate, onOpenCalendar, onOpenChat, chatUnread }: Props) {
  ...
  return (
    <header className="sticky top-0 z-40 px-6 py-3 flex items-center justify-between"
      style={{ background: dark ? 'rgba(7,12,20,0.92)' : 'rgba(240,244,248,0.92)', backdropFilter: 'blur(20px)', borderBottom: `1px solid ${dark ? 'rgba(255,255,255,0.07)' : '#dde3ec'}` }}>
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className="font-display font-bold text-sm" style={{ color: dark ? '#f1f5f9' : '#0d1b2a' }}>ExecTrack</div>
          <div className="text-xs" style={{ color: dark ? '#475569' : '#94a3b8' }}>CEO Executive Tracker</div>
        </div>
      </div>

      {/* Center — view toggle + date */}
      <div className="flex items-center gap-3">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-medium" style={{ color: dark ? '#64748b' : '#94a3b8' }}>{connected ? 'Live' : 'Offline'}</span>
        </div>
          ...
          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold"
            ...
            {others.map(u => (
              <span key={u.userId} className="flex items-center gap-1">
                <span className="inline-flex w-5 h-5 rounded-full items-center justify-center text-white font-bold"
        ...
        <button onClick={onOpenChat}
          ...
          {chatUnread > 0 && (
            <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full text-white text-xs flex items-center justify-center font-bold"
              style={{ background: '#ef4444', fontSize: '9px' }}>{chatUnread}</span>
          )}
        </button>
