...
export default function LoginPage() {
  ...
  return (
    <div className={`min-h-screen flex ${mounted ? '' : 'invisible'}`}
      style={{ background: dark ? 'linear-gradient(135deg,#070c14 0%,#0d1a2e 50%,#091524 100%)' : 'linear-gradient(135deg,#e8edf5 0%,#dce5f0 100%)' }}>
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-1/4 right-1/4 w-80 h-80 rounded-full blur-3xl opacity-10" style={{ background: '#c89b3c' }} />
      </div>
      ...
      <div className="hidden lg:flex flex-col justify-between w-1/2 p-14 relative z-10">
        <div>
          <div className="flex items-center gap-3 mb-16">
            ...
            <span className="font-display font-bold text-lg" style={{ color: dark ? '#f1f5f9' : '#163a63' }}>ExecTrack</span>
          </div>
          <h1 className="font-display text-5xl font-bold leading-tight mb-6" style={{ color: dark ? '#f1f5f9' : '#0d1b2a' }}>
            Executive Command<br /><span style={{ color: dark ? '#f59e0b' : '#c89b3c' }}>Centre.</span>
          </h1>
          <p className="text-lg leading-relaxed mb-10" style={{ color: dark ? '#94a3b8' : '#64748b' }}>
            Real-time executive task management with live collaboration, calendar, and instant messaging.
          </p>
        </div>
        <div className="space-y-4">
          {[
            { icon: '⚡', text: 'Live real-time sync — no page refresh needed' },
            { icon: '📅', text: 'Calendar view — see any past day\'s work' },
            { icon: '💬', text: 'Live chat between Mr. Mohammed & Darlene' },
            { icon: '📄', text: 'One-click PDF report generation' },
          ].map(f => (
        ...
        <p className="text-sm" style={{ color: dark ? '#475569' : '#94a3b8' }}>© 2026 CEO Executive Tracker. Confidential.</p>
      </div>
        ...
        <div className="w-full max-w-md">
          <div className="rounded-2xl p-8 shadow-2xl"
            ...
            <div className="flex gap-3 mb-6">
              <button onClick={() => quickLogin('mohammed', process.env.NEXT_PUBLIC_CEO_HINT || 'ceo2026')}
                className="flex-1 py-3 rounded-xl text-sm font-semibold border transition-all hover:scale-[1.02]"
                style={{ background: dark ? 'rgba(59,130,246,0.12)' : '#eff6ff', color: '#3b82f6', borderColor: dark ? 'rgba(59,130,246,0.3)' : '#bfdbfe' }}>
                👔 Mr. Mohammed
              </button>
              <button onClick={() => quickLogin('darlene', process.env.NEXT_PUBLIC_ASSISTANT_HINT || 'assist2026')}
                className="flex-1 py-3 rounded-xl text-sm font-semibold border transition-all hover:scale-[1.02]"
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold uppercase tracking-wider mb-1.5" style={{ color: dark ? '#64748b' : '#94a3b8' }}>Username</label>
                <input value={username} onChange={e => setUsername(e.target.value)} placeholder="mohammed or darlene"
              ...
              <button type="submit" disabled={loading || !username || !password}
                className="w-full py-3 rounded-xl font-semibold text-white transition-all hover:scale-[1.01] disabled:opacity-50 mt-2"
